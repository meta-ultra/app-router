...
import nomalizeOutput from "./dynamic-routes/normalizeOutput";
...
test("staticDefaultImports with hash router", () => {
  ...
  expect(output).toStrictEqual(
    readFileSync(join(__dirname, "./dynamic-routes/staticDefaultImportsOutput.hash.hbs")).toString(
      "utf-8"
    )
  );
});
...
test("staticDefaultImports with browser router", () => {
  ...
  expect(output).toStrictEqual(
    readFileSync(join(__dirname, "./dynamic-routes/staticDefaultImportsOutput.browser.hbs")).toString(
      "utf-8"
    )
  );
});
